...
policy_type_list = ["DensePolicy"]  # "DenseParallel",
ran = 1  # 0--14
test = "test2"  # 测试序号，test1,2,3,4...
seed_num = 1  # 随机数种子，0,1,2...
profit_array = []
percent_array = []
for j in range(0, 1):
    profit = []
    ...
    env = StockEnvPlot.StockEnv(
        type='test', ran=ran)
    env = DummyVecEnv([lambda: env])
    model = PPO2.load("./ModelSave/" +
                      policy_type_list[policy_type_number] + "/" + "PPO2" + stock_list[ran] + "_" + policy_type_list[
                          policy_type_number] + "_" +
                      test + "_seed" + str(seed_num) + "_163840_steps.zip",
                      env, seed=0)
